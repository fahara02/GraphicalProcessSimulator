...
class Display
{
  public:
	...
	ScreenRotation getScreenRotation() const { return static_cast<ScreenRotation>(rotation_); }
  ...
  private:
	...
	void drawMenuFramework()
	{
		...
		bg_->fillRoundRect(10, 10, bg_->width(), bg_->height() - 20, menuMetrics.radius,
						   Colors::menu_bg);
		bg_->drawRoundRect(10, 10, bg_->width() - 20, bg_->height() - 20, menuMetrics.radius,
						   Colors::frame);
	...
	void drawMenuItem(int16_t yPos, size_t itemIndex)
	{
		...
		bg_->fillRoundRect(20, yPos, itemWidth, menuMetrics.textHeight, menuMetrics.radius,
		...
		if(isSelected)
		{
			bg_->drawRoundRect(20, yPos, itemWidth, menuMetrics.textHeight, menuMetrics.radius,
							   Colors::cursor); // Yellow border
		}
	}
	...
	void drawTriangle(int16_t x, int16_t y, bool up)
	{
		...
		bg_->fillTriangle(x - size, y + (up ? size : 0), x + size, y + (up ? size : 0), x,
						  y + (up ? 0 : size), Colors::scroll_indicator);
	}
