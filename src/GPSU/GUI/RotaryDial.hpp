...
namespace GUI
{
template<size_t N, size_t P = 60>
class RotaryDial
{
  public:
	RotaryDial(TFT_eSprite& sprite, const std::array<String, N>& labels, int cx, int cy, int r) :
	...
	void draw()
	{
		...
		dialSprite.fillCircle(centerX, centerY, 124, static_cast<uint16_t>(Colors::dial_bg));
		...
		for(size_t i = 0; i < P; i++)
		{
			...
			dialSprite.fillCircle(px[idx], py[idx], 1, static_cast<uint16_t>(Colors::dial_points));
		}

		// Draw indicator triangle
		dialSprite.fillTriangle(centerX - 1, centerY - 70, centerX - 5, centerY - 56, centerX + 4,
								centerY - 56, TFT_ORANGE);
		lastAngle = angle;
	}
