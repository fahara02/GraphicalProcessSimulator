...
Display::Display(const MenuItem* items, size_t count) :
	...
	canvas_(std::make_unique<TFT_eSPI>()), bg_(std::make_unique<TFT_eSprite>(canvas_.get())),
	label_(std::make_unique<TFT_eSprite>(canvas_.get())),
	layer_1(std::make_unique<TFT_eSprite>(canvas_.get())),
	layer_2(std::make_unique<TFT_eSprite>(canvas_.get())), current_process_(GPSU::ProcessType::ANY)
{
}
...
void Display::updateStepperDisplay(const int dir, int step)
{
	...
	layer_2->fillRoundRect(stator_x, stator_y, stator_length, stator_width, 20, TFT_BLUE);
