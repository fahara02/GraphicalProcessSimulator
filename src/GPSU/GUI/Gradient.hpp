...
{
	return ((static_cast<uint16_t>(r & 0xF8) << 8) | (static_cast<uint16_t>(g & 0xFC) << 3) |
			(b >> 3));
}
...
template<uint16_t StartColor, uint16_t EndColor>
struct ColorInterpolator
{
	constexpr uint16_t operator()(size_t i, size_t steps) const
	{
		...
		long r = static_cast<long>(startR) + (diffR * step + total_steps / 2) / total_steps;
		long g = static_cast<long>(startG) + (diffG * step + total_steps / 2) / total_steps;
		long b = static_cast<long>(startB) + (diffB * step + total_steps / 2) / total_steps;
		...
		return color565(static_cast<uint8_t>(r), static_cast<uint8_t>(g), static_cast<uint8_t>(b));
	}
};
...
struct SymmetricGradientColorFunc
{
	constexpr uint16_t operator()(size_t i, size_t steps) const
	{
		...
		else if(i <= half)
		{
			intensity =
				minIntensity +
				static_cast<uint8_t>((static_cast<long>(maxIntensity - minIntensity) * i) / half);
		}
		else
		{
			intensity =
				minIntensity +
				static_cast<uint8_t>(
					(static_cast<long>(maxIntensity - minIntensity) * (steps - 1 - i)) / half);
		}
		return color565(intensity, intensity, intensity);
	}
};
...
constexpr auto itemSelectedBgGradient =
	generateGradient<50>(ColorInterpolator<ITEM_SELECTED_START, ITEM_SELECTED_END>{});
...
static inline void drawCustomGradient(TFT_eSPI* tft, int x, int y, int w, int h,
