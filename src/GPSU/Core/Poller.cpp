...
template class Poller<IO_Controller>;

template <typename T> Poller<T>::~Poller() {
  if (pollTimerhandler) {
...
template <typename T> bool Poller<T>::isPolling() const {
  if (pollTimerhandler && xTimerIsTimerActive(pollTimerhandler) != pdFALSE)
    return true;

  return false;
}
template <typename T> bool Poller<T>::autopoll(bool newstate) {
  if (newstate) {
    // Check if the timer handler is null and create the timer if needed
    if (!pollTimerhandler) {
      pollTimerhandler = xTimerCreate(
          POLLER_NAME,                      // Name of the poller
          pdMS_TO_TICKS(poll_period),       // Period of the timer in ticks
          pdTRUE,                           // Auto-reload
          reinterpret_cast<void *>(Device), // Timer ID
          &GPSU_CORE::Poller<T>::timeKeeper // Callback function
      );
...
template <typename T> size_t Poller<T>::getPollrate() const {
  if (pollTimerhandler)
    return pdTICKS_TO_MS(xTimerGetPeriod(pollTimerhandler));
  return 0;
}
template <typename T> bool Poller<T>::setPollrate(size_t pollTime) {
  if (pollTime < POLLER_MIN_PERIOD)
    return false;

  if (xTimerChangePeriod(pollTimerhandler, pollTime / portTICK_PERIOD_MS,
                         TIMER_CMD_TIMEOUT) == pdPASS)
    return true;
