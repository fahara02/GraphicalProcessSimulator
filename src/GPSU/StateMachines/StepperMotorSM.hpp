...
struct Traits
{
	...
	static inline bool move(const Context& ctx) { return ctx.inputs.ui.start_move; }
	static inline bool cw(const Context& ctx) { return ctx.inputs.sensors.direction_state; }
	static inline bool cw_limit(const Context& ctx) { return ctx.inputs.sensors.cw_limit; }
	static inline bool ccw_limit(const Context& ctx) { return ctx.inputs.sensors.ccw_limit; }
	static inline bool home(const Context& ctx) { return ctx.inputs.ui.home_request; }
	...
	static inline bool stop_requested(const Context& ctx) { return ctx.inputs.ui.stop; }
	static inline bool home_complete(const Context& ctx) { return ctx.inputs.sensors.home_switch; }
	static inline bool clear_error(const Context& ctx) { return ctx.inputs.ui.clear_error; }
	struct entryActions
	{
	...
	static constexpr std::array<Transition, transition_count> transitions = {{
		...
		{State::IDLE, State::MOVING_CCW, move_ccw, entryActions::start_move_ccw,
		 exitActions::stop_motor},
		{State::IDLE, State::HOMING, home, entryActions::start_homing, exitActions::stop_motor},
		{State::IDLE, State::ERROR, fault_detected, entryActions::emergency_stop, nullptr},

		// From MOVING_CW
		{State::MOVING_CW, State::IDLE, movement_complete, nullptr, exitActions::stop_motor},
		{State::MOVING_CW, State::IDLE, stop_requested, nullptr, exitActions::stop_motor},
		{State::MOVING_CW, State::ERROR, fault_detected, entryActions::emergency_stop,
		 exitActions::stop_motor},
		{State::MOVING_CW, State::ERROR, ccw_limit, entryActions::emergency_stop,
		 exitActions::stop_motor},

		// From MOVING_CCW
		{State::MOVING_CCW, State::IDLE, movement_complete, nullptr, exitActions::stop_motor},
		{State::MOVING_CCW, State::IDLE, stop_requested, nullptr, exitActions::stop_motor},
		{State::MOVING_CCW, State::ERROR, fault_detected, entryActions::emergency_stop,
		 exitActions::stop_motor},
		{State::MOVING_CCW, State::ERROR, cw_limit, entryActions::emergency_stop,
		 exitActions::stop_motor},

		// From HOMING
		{State::HOMING, State::IDLE, home_complete, nullptr, exitActions::stop_motor},
		{State::HOMING, State::ERROR, fault_detected, entryActions::emergency_stop,
		 exitActions::stop_motor},
...
class StepperMotorSM : public StateMachine<StepperMotor::Traits, StepperMotorSM>
{
  public:
	...
	StepperMotorSM(const Context& context) : StepperMotorSM(context, State::IDLE) {}
