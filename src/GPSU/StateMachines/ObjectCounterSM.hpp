...
struct Traits
{
	...
	static inline bool eStop(const Context& ctx) { return ctx.event == Event::ESTOP; }
	static inline bool faultCleared(const Context& ctx) { return ctx.event == Event::OK; }
	...
	{
		return (ctx.mode == Mode::MANUAL && ctx.inputs.ui.start && !ctx.inputs.ui.stop);
	}

	static inline bool start(const Context& ctx) { return auto_mode(ctx) || manual_start(ctx); }
...
namespace SM
{
class ObjectCounterSM : public StateMachine<ObjectCounter::Traits, ObjectCounterSM>
{
  public:
	...
	void updateInternalState(const Inputs& input)
	{
		...
		bool mode_changed = (ctx_.inputs.ui.manual_mode ? Mode::MANUAL : Mode::AUTO) != ctx_.mode;
		...
		if(mode_changed && ctx_.mode == Mode::MANUAL && current() == State::RUNNING)
		{
  ...
  private:
	...
	void addObject(uint32_t runtime)
	{
		...
		item.on_conv = true;
		item.data = {runtime, runtime + ctx_.config.sense_delay, runtime + ctx_.config.pick_delay,
					 0, 0};
	...
	void updateObjects(uint32_t runtime)
	{
		...
		for(uint8_t i = 0; i < ctx_.obj_cnt; i++)
		{
			...
			// Update x_position
			if(item.state != Items::State::PICKED && item.state != Items::State::FAILED)
			{
			...
			if(!item.sensed)
			{
				...
				else if(item.state != Items::State::SENSED || runtime >= ctx_.config.sense_timeout)
				{
	...
	void pickerProcessing(Item& obj, uint32_t runtime)
	{
		...
		obj.data.deadline = runtime + (ctx_.mode == Mode::AUTO ? ctx_.config.auto_timeout :
