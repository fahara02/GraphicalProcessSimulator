...
constexpr std::size_t compute_state_count(const std::array<Transition, N>& transitions)
{
...
class StateMachine
{
  public:
	...
	explicit StateMachine(const Context& ctx, State initial, bool enableTimer,
		...
		ctx_(ctx), current_(initial), previous_(initial), enable_timer_(enableTimer),
		enable_timer2_(enableTimer2)
	{
	}
	~StateMachine()
	{
  ...
  private:
	...
	esp_err_t startTimer(uint64_t timeout_us, int timer_id = 0)
	{
		if(timer_id == 0)
		{
			return enable_timer_ ? esp_timer_start_once(timer_, timeout_us) : ESP_FAIL;
		}
		return enable_timer2_ ? esp_timer_start_once(timer2_, timeout_us) : ESP_FAIL;
	}
