...
struct Register {
  ...
  Register(MCP_MODEL m = MCP_MODEL::MCP23017, MCP::REG rg = MCP::REG::IODIR,
           MCP::PORT p = MCP::PORT::GPIOA, bool bankMode = false,
           bool readonly = false)
      : model(m), reg(rg), port(p), readOnly_(readonly),
        bankSeparated_(bankMode), value_(0),
  ...
  uint8_t getSavedValue() const {
    ...
    return reg == REG::IOCON ? config_.getSettingValue() : value_;
  }
