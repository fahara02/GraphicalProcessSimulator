...
namespace GPSU
{
StackType_t Process::sharedStack[Process::process_task_depth]; // Define the stack array
StaticTask_t Process::sharedTCB;
const MenuItem Process::process_list[] = {{"TrafficLight", &Process::processTrafficLight},
...
Process::Process() :
	sda_(GPIO_NUM_25), scl_(GPIO_NUM_33), reset_(GPIO_NUM_13), pinA_(GPIO_NUM_37),
	pinB_(GPIO_NUM_38), btn_(GPIO_NUM_32),
	display_(GUI::Display::getInstance(process_list, process_count)),
	io_(std::make_shared<COMPONENT::MCP23017>(sda_, scl_, reset_)),
	menu_(std::make_unique<MenuSelector>(process_list, process_count, 4, pinA_, pinB_, btn_)),
...
void Process::handleSelectionChanged(size_t index)
{
	if(current_process_ != ProcessType::ANY)
	{
		...
		if(processTaskHandle != nullptr)
		{
			xTaskNotify(processTaskHandle, TERMINATE_NOTIFICATION, eSetValueWithOverwrite);
...
ObjectCounter::Context GPSU::Process::mapUserCommand(ObjectCounter::Context ctx)
{
	uint8_t pinStatus = io_->digitalRead(MCP::PORT::GPIOB);
...
void Process::traffic_light_task(void* param)
{
	...
	while(true)
	{
		...
		instance->mapOutputs<ProcessType::TRAFFIC_LIGHT, TrafficLight::Context>(ctx);

		const char* current = GPSU::Util::ToString::TLState(instance->tlsm_->current());
		const char* previous = GPSU::Util::ToString::TLState(instance->tlsm_->previous());
